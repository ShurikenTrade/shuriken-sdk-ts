...
describe('suggestions', () => {
  ...
  describe('dismiss', () => {
    it('POSTs to /:id/dismiss with reason in body', async () => {
      fetchSpy = mockFetch(200, {
        data: { ...SAMPLE_SUGGESTION, state: 'DISMISSED', dismissReason: 'too risky' },
      })
      vi.stubGlobal('fetch', fetchSpy)

      const result = await createClient().suggestions.dismiss(SAMPLE_SUGGESTION.id, 'too risky')

      expect(result.state).toBe('DISMISSED')
      expect(result.dismissReason).toBe('too risky')

      const [url, init] = fetchSpy.mock.calls[0]
      expect(url).toBe(`${BASE_URL}/api/v2/agents/suggestions/${SAMPLE_SUGGESTION.id}/dismiss`)
      expect(init?.method).toBe('POST')
      expect(JSON.parse(init?.body as string)).toEqual({ reason: 'too risky' })
    })

    it('omits reason when absent', async () => {
      fetchSpy = mockFetch(200, { data: { ...SAMPLE_SUGGESTION, state: 'DISMISSED' } })
      vi.stubGlobal('fetch', fetchSpy)

      await createClient().suggestions.dismiss(SAMPLE_SUGGESTION.id)

      const [, init] = fetchSpy.mock.calls[0]
      expect(JSON.parse(init?.body as string)).toEqual({})
    })

    it('handles 409 SUGGESTION_NOT_OPEN error envelope', async () => {
      fetchSpy = mockFetch(409, {
        error: { code: 'SUGGESTION_NOT_OPEN', message: 'suggestion is not open' },
        requestId: 'req_xyz',
      })
      vi.stubGlobal('fetch', fetchSpy)

      const promise = createClient().suggestions.dismiss(SAMPLE_SUGGESTION.id)
      await expect(promise).rejects.toBeInstanceOf(ShurikenApiError)
      await expect(promise).rejects.toMatchObject({
        status: 409,
        apiCode: 'SUGGESTION_NOT_OPEN',
        requestId: 'req_xyz',
      })
    })
  })
  ...
  describe('ack', () => {
    it('POSTs to /:id/ack with linkedTaskId in body', async () => {
      fetchSpy = mockFetch(200, {
        data: { ...SAMPLE_SUGGESTION, state: 'ACTED', linkedTaskId: 'task_999' },
      })
      vi.stubGlobal('fetch', fetchSpy)

      const result = await createClient().suggestions.ack(SAMPLE_SUGGESTION.id, 'task_999')

      expect(result.state).toBe('ACTED')
      expect(result.linkedTaskId).toBe('task_999')

      const [url, init] = fetchSpy.mock.calls[0]
      expect(url).toBe(`${BASE_URL}/api/v2/agents/suggestions/${SAMPLE_SUGGESTION.id}/ack`)
      expect(init?.method).toBe('POST')
      expect(JSON.parse(init?.body as string)).toEqual({ linkedTaskId: 'task_999' })
    })

    it('omits linkedTaskId when absent', async () => {
      fetchSpy = mockFetch(200, { data: { ...SAMPLE_SUGGESTION, state: 'ACTED' } })
      vi.stubGlobal('fetch', fetchSpy)

      await createClient().suggestions.ack(SAMPLE_SUGGESTION.id)
